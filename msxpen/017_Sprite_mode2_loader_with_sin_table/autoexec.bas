...
4 '
5 ' --------------------------------------------------------------------------
6 ' 017_Sprite_mode2_loader_with_sin_table
7 ' --------------------------------------------------------------------------
8 '
9 DIM SI%(32):DIM CO%(32):T%=0
10 CLS:SCREEN 5,2:COLOR 15,0,1:OPEN "GRP:" FOR OUTPUT AS #1
20 '-- LOAD Back Ground
21 'BLOAD "PSD3D4.SC5",S:COLOR=RESTORE
30 '-- LOAD SPRITE GENERATOR & COLOR
31 '_TURBOON
32 GOSUB 910:GOSUB 921:GOSUB 22001
40 '-- INITILIZE X & Y
41 X(0)=80:Y(0)=0
50 'DISPLAY SPRITE
51 GOSUB 850
52 '_TURBOON
53 ON INTERVAL=60 GOSUB 721
54 INTERVAL ON
55 '_TURBOOFF
60 'GOTO MAIN ROUTINE
61 GOTO 61
...
745 T%=T%+1
746 A=&H7600
747 'IF INKEY$="" THEN 747
777 X=X(0):Y=Y(0):FOR I=0 TO 2:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
778 X=16+X(0):Y=Y(0):FOR I=0 TO 2:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
779 X=X(0):Y=16+Y(0):FOR I=0 TO 2:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
780 X=16+X(0):Y=16+Y(0):FOR I=0 TO 2:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
781 X=X(0):Y=32+Y(0):FOR I=0 TO 2:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
782 X=16+X(0):Y=32+Y(0):FOR I=0 TO 1:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
783 X=X(0):Y=48+Y(0):FOR I=0 TO 1:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
784 X=16+X(0):Y=48+Y(0):FOR I=0 TO 1:VPOKE A,Y:VPOKE A+1,X:A=A+4:NEXT I
795 RETURN
850 '-- DISPLAY SPRITE
...
923 S1%=S0%:B$=CHR$(VAL(A$)):FOR I=1 TO 31:READ A$:B$=B$+CHR$(VAL(A$)):NEXT:SPRITE$(S0%)=B$:S0%=S0%+1
930 'LOAD SPRITE COLOR
931 'SN%=0':S=&H7400
932 READ A$: IF A$="*" THEN RETURN
933 B$=CHR$(VAL(A$)):FOR I=1 TO 15:READ A$:B$=B$+CHR$(VAL(A$)):NEXT:COLOR SPRITE$(S1%)=B$:S1%=S1%+1
...
13830 DATA *
22000 '-- LOAD SI TABLE
22001 RESTORE 24003:FOR I=0 TO 31:READ A$:CO%(I)=VAL("&H"+A$):NEXT
22002 RESTORE 25003:FOR I=0 TO 31:READ A$:SI%(I)=VAL("&H"+A$):NEXT
22100 RETURN
24002 REM COS TABLE : 0-90 (32 STEPS) 
...
25002 REM SIN TABLE : 0-90 (32 STEPS) 
