...
4 '
5 ' --------------------------------------------------------------------------
6 ' MODE2 SPRITE LOADER Example for tinysprite
7 ' --------------------------------------------------------------------------
8 '
...
23 'CALL TURBO ON:DIM A(8):DIM B(8):DIM X(8):DIM Y(8):DIM P(16):DIM Q(16)
30 '-- LOAD SPRITE GENERATOR & COLOR
31 GOSUB 910:GOSUB 921:
50 'DISPLAY SPRITE
52 X(0)=128-16:Y(0)=98-16
53 GOSUB 850
54 'CALL TURBO OFF
55 'ON INTERVAL=60 GOSUB 741
56 'INTERVAL ON
57 'CALL TURBO ON
58 REM
70 'GOTO MAIN ROUTINE
71 GOTO 2030
740 '-- UPDATE X & Y of SPRITES
741 S=STICK(0) 
742 IF S=1 THEN Y(0)=Y(0)-1 
743 IF S=3 THEN X(0)=X(0)+1 
744 IF S=5 THEN Y(0)=Y(0)+1 
745 IF S=7 THEN X(0)=X(0)-1
746 A=&H7600
...
13830 DATA *
13840 REM -- MSX2 LOADER NOT YET IMPLEMENTED